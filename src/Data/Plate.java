...
public class Plate {
	...
	private ArrayList<Plate> adjacencies;
	...
	public Plate(Polygon start, boolean majorPlate) {
		...
		// Colour =
		// Main.ColourPalette.Colours[rand.nextInt(Main.ColourPalette.Colours.length)];
		...
		for (Polygon p : this.polys) {
			if (p.isEdgeOfPlate()) {
				for (Polygon q : p.getAdjacencies()) {
					if (q.getPlate() != this) {
						found = false;
						for (Plate P : this.adjacencies) {
							if (q.getPlate() == P) {
								found = true;
							}
						}
						if (!found) {
							this.adjacencies.add(q.getPlate());
						}
					}
				}
			}
	...
	public ArrayList<Plate> getAdjacencies() {
